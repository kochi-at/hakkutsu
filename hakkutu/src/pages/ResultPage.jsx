...
import { digTier } from "../components/digTier";
...
function RarityStars({ rarity }) {
  return (
    <span className="relic-card-rarity" role="img" aria-label={`星評価 ${rarity.toFixed(1)} / 5`}>
      ...
      <span
        className="relic-card-rarity-filled"
        aria-hidden="true"
        style={{ width: `${(rarity / 5) * 100}%` }}
      >
        ★★★★★
      </span>
    </span>
        ...
        <div className="relic-card-inner" data-element={evaluation.element}>
          ...
          <div className="relic-card-header">
            <span className="relic-card-badge">
              <span className="relic-card-badge-icon">
                <ElementIcon element={evaluation.element} />
              </span>
          ...
          <h1 className="relic-card-name">
            <RubyText parts={evaluation.name_parts} fallback={evaluation.name} />
          </h1>

          {state.photo && (
            <div className="relic-card-art">
              <img src={state.photo} alt="鑑定した写真" />
              {sparkle === "sheen" && <div className="relic-card-sheen" aria-hidden="true" />}
              {sparkle === "holo" && (
                <>
                  <div className="relic-card-holo" aria-hidden="true" />
                  <div className="relic-card-holo-grain" aria-hidden="true" />
                </>
              )}
            </div>
          )}
          ...
          <div className="relic-card-stats">
            {Object.entries(STAT_LABELS).map(([key, label]) => (
              <div className="relic-card-stat" key={key}>
                ...
                <span className="relic-card-stat-bar">
                  <span
                    className="relic-card-stat-fill"
          ...
          <div className="relic-card-lore">
            ...
            <p className="relic-card-lore-text">
              <RubyText parts={evaluation.lore_parts} fallback={evaluation.lore} />
            </p>
          </div>

          <div className="relic-card-footer">
            <span className="relic-card-tier">{tier}</span>
            <span className={`relic-card-seal relic-card-seal--${sparkle}`} aria-hidden="true">
              <SealIcon />
            </span>
          </div>
